...
- Since TryItOnline security is based on selinux, and it is not possible to run selinux specific commands inside a docker container, selinux is disabled in this image. This means, that if someone can browse to the web site served by this image, the can potentially compromise the host. **!!!Do not expose installations based on this image to any non-trused enviroment (e.g.: internet)!!!**
